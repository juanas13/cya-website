// app/ClientLayout.tsx  ← C'EST ICI que va tout le code avec useState
"use client";
...
export default function ClientLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <>
      {/* NAVBAR FIXE */}
      <nav className="fixed top-0 left-0 right-0 z-50 bg-black/90 backdrop-blur-xl border-b border-purple-800/50 shadow-2xl">
        <div className="max-w-7xl mx-auto px-6 py-5 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-4 group">
            <Image
              ...
              width={60}
              height={60}
              className="rounded-full border-4 border-purple-600 shadow-xl group-hover:scale-110 transition"
              priority
            />
          ...
          <div className="hidden md:flex items-center gap-10 text-lg font-medium">
            ...
            <Link
              ...
              target="_blank"
              className="px-6 py-3 bg-gradient-to-r from-cyan-500 to-purple-600 rounded-full font-bold flex items-center gap-2"
            >
      ...
      {isMobileMenuOpen && (
        <>
          <div className="fixed inset-0 bg-black/80 z-40" onClick={() => setIsMobileMenuOpen(false)} />
          <div className="fixed inset-y-0 right-0 w-80 bg-black/95 backdrop-blur-xl border-l border-purple-800 z-50 flex flex-col">
            <div className="flex-1 overflow-y-auto px-8 pt-24 pb-12">
              <div className="flex flex-col space-y-8">
                <Link href="/" onClick={() => setIsMobileMenuOpen(false)} className="text-2xl font-bold hover:text-purple-400 py-3">Accueil</Link>
                <Link href="/staking" onClick={() => setIsMobileMenuOpen(false)} className="text-2xl font-bold hover:text-purple-400 py-3">Staking</Link>
                <Link href="/whitepaper" onClick={() => setIsMobileMenuOpen(false)} className="text-2xl font-bold hover:text-purple-400 py-3">Whitepaper</Link>
                <Link href="/buy" onClick={() => setIsMobileMenuOpen(false)} className="text-2xl font-bold hover:text-purple-400 py-3">Acheter</Link>
                <Link
                  href="https://pancakeswap.finance/swap?outputCurrency=0x671cb1a2c934017fc019a7a4bf11ae5a30a32354"
                  target="_blank"
      ...
      <footer className="bg-gray-950 border-t border-purple-900 py-12 text-center text-gray-400">
        © 2025 CYA Token – Tous droits réservés<br />DYOR • Not financial advice
      </footer>
    </>
  );
}
